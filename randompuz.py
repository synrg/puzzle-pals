...
def choose_random_image_url(low, high):
    image_number = random.randint(low, high)
    return f'{BASE_URL}/browse/image/{image_number}'
...
def get_random_image_url(session, lowest, highest):
    errors = 0
    urls_tried = []
    valid_url = None
    size_of_range = highest - lowest + 1
    max_errors = min(MAX_ERRORS, size_of_range)
    while not valid_url and errors < max_errors and len(urls_tried) < size_of_range:
        url = choose_random_image_url(lowest, highest)
    ...
    if not valid_url:
        sys.exit(f"Gave up after {errors} retries. Site down or no valid images in range: {lowest}-{highest}")
    return valid_url

def main():
    session = requests_cache.CachedSession(CACHE_FILE, expire_after=timedelta(days=1))
    lowest, highest = get_params(session)
    url = get_random_image_url(session, lowest, highest)
    print(f"\nRandom puzzle image: {url}\n")
